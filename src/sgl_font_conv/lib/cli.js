...
module.exports.run = async function (argv, debug = false) {
  ...
  let parser = new argparse.ArgumentParser({
    ...
    epilog: 'For example: \n./sgl_font_conv.js --font YaHei.ttf --range 0x20-0x7F --range 0x5010-0x5010 --range 0x5030-0x5030 --size 24 --bpp 4 --output font_output/sgl_font_test.c\n'
  });

  // Save original format_usage
  const originalFormatUsage = parser.format_usage.bind(parser);
...
`
  ...
  parser.add_argument('--compress', {
    ...
    action: 'store_true',
    default: false,
    help: 'use compression, default is off.'
  });
  ...
  parser.add_argument('--align', {
    ...
    type: positive_int,
    default: 1,
    help: 'Align each bitmap address to the specified number of bytes.'
  });

  // parser.add_argument('--sgl-include', {
  //   metavar: '<path>',
  //   help: 'Set alternate "sgl.h" path (for --format sgl).'
  // });

  // parser.add_argument('--sgl-font-name', {
  //   help: 'Variable name of the sgl font structure. Defaults to the output\'s basename.'
  // });
  ...
  if (args.stride > 0) {
    if (args.compress === false) {
      parser.error('--stride requires --compress');
    }
